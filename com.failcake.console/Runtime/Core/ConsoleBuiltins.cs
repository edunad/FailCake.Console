...
#region STATIC
		...
		private static void CvarList() {
			string text = string.Join("\n", ConsoleRegistry.GetAll().OfType<ConsoleVar>().Where(ConsoleBuiltins.IsVisible).Select(ConsoleBuiltins.FormatEntry));
		...
		#if !UNITY_SERVER
		[Command("exec", "Execute a local cfg file")]
		...
		#if UNITY_SERVER
		[Command("host_writeconfig", "Save archived cvars to config.cfg", FCVAR.ADMIN)]
		#else
